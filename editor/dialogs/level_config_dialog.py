...
class LevelConfigDialog(Dialog):
    ...
    def __init__(self, level, gui_atlas):
        ...
        super().__init__(pos, (width, height), gui_atlas.load_sliced("window_bkg_large"),
                         font=font, title=title, tb_bkg=gui_atlas.load_sliced("tb_frame"), additional_height=8,
        ...
        self.bkg_color_r_slider = create_slider(gui_atlas,
                                                make_vector(slider_left,
                                                            bkg_color_text.relative_position.y + bkg_color_text.height),
                                                slider_width, 0, 255, self._on_slider_changed)

        # slider for g
        g_text = Text(make_vector(frame_width, self.bkg_color_r_slider.relative_position.y +
                                  self.bkg_color_r_slider.height),
                      text="G:", font=font, text_color=black)

        self.add_child(g_text)

        self.bkg_color_g_slider = create_slider(gui_atlas,
                                                make_vector(slider_left, r_text.relative_position.y + g_text.height),
                                                slider_width, 0, 255, self._on_slider_changed)

        # slider for b
        b_text = Text(make_vector(frame_width,
                                  self.bkg_color_g_slider.relative_position.y + self.bkg_color_g_slider.height),
                      text="B:", font=font, text_color=black)

        self.add_child(b_text)

        self.bkg_color_b_slider = create_slider(gui_atlas,
                                                make_vector(slider_left, g_text.relative_position.y + b_text.height),
                                                slider_width, 0, 255, self._on_slider_changed)
        ...
        # buttons
        pos = make_vector(3 * frame_width // 2,
                          self.bkg_color_b_slider.height + self.bkg_color_b_slider.relative_position.y + 6)
        button_y_offset = 4

        button_size = (self.width - 3 * frame_width, self.bkg_color_b_slider.height * 1.5)

        self.save_button = create_button(gui_atlas, pos, button_size, "Save", font=font,
                                         on_click_callback=self._on_save_map)
        self.add_child(self.save_button)

        pos.y += self.save_button.height + button_y_offset

        self.load_button = create_button(gui_atlas, pos, button_size, "Load", font=font,
                                         on_click_callback=self._on_load_map)
        self.add_child(self.load_button)

        pos.y += self.save_button.height + button_y_offset

        self.width_button = create_button(gui_atlas, pos, button_size, "Set Width", font=font,
                                          on_click_callback=self._on_change_width)
        self.add_child(self.width_button)

        pos.y += self.save_button.height + button_y_offset

        self.height_button = create_button(gui_atlas, pos, button_size, "Set Height", font=font,
                                           on_click_callback=self._on_change_height)
        self.add_child(self.height_button)

        pos.y += self.save_button.height + button_y_offset

        # level dimensions
        self.level_dimensions = Text(pos, f"Dimensions: {self.level.tile_map.width}x{self.level.tile_map.height}",
                                     font, config.default_text_color)
